...
3. **预期条数前置断言**：涉及"批量填充 N 行"或"对 M 个区域分别写入"时，先把 N、M 硬编码进代码，回读后断言实际等于预期；不一致时先根据回读结果计算缺失 / 失败子集，再用只包含该子集的新 operations 调 `+batch-update` 补齐。禁止复用原始整批 payload，也禁止交付半成品。
