...
# 多行、结构化或非 ASCII patch 先保存为当前 CWD 下的 UTF-8 文件
lark-cli docs +update --doc "xx" --command block_replace --block-id blkTarget --content "@./patch.xml"
lark-cli docs +update --doc "xx" --command block_replace --start-block-id blkFirst --end-block-id blkLast --content "@./patch.xml"

lark-cli docs +update --doc "xx" --command block_insert_after --block-id blkAnchor --content "@./insert.xml"
...
4. **Patch（精确修改）**：按 block / section 执行局部命令。短小的行内 `str_replace` 可直接传值；多行、结构化或非 ASCII patch 通过以命令 CWD 为根的文件工具写成 UTF-8 文件，再用相对 `@file` 传入。Windows 下不得使用 PowerShell 文本管道向 `--content -` 传 patch；路径失败时修正命令执行 CWD。替换内容必须符合目标父容器的结构；例如替换列表项范围时使用 `<li>...</li>`。保护 `<cite>`、`<img>`、`<source>`、`<whiteboard>`、`<sheet>`、`<bitable>`、`<synced_reference>` 等 token 化内容，不要改成纯文本或占位符。同一 block 的多处修改合并成一次 `block_replace`。
...
|`--content`|视指令|写入内容；Agent 的多行、结构化或非 ASCII patch 使用当前 CWD 下的 UTF-8 相对 `@file`；`str_replace` 传空字符串可删除文本|
...
|`--reference-map`|否|保留或回放既有 `reference_map`，需与 `--content` 配合；Agent 使用任务 CWD 内的 UTF-8 相对 `@file`；也支持直接 JSON，stdin `-` 仅供能直接提供原始 UTF-8 字节的宿主调用方使用|
