...
## Shell 传参
- **首选文件传参**：Agent 将 Markdown 以 UTF-8 保存到命令 CWD，并使用 `@./path/to/file.md`（读文件），以绕开 shell 转义和 Windows 文本管道转码；`-`（读 stdin）仅供能直接提供原始 UTF-8 字节的宿主调用方使用。字面量以 `@` 开头时用 `@@` 转义（`--pattern` 不支持 `@file`）
