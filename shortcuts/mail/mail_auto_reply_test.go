...
func TestMailAutoReplyProjectsImageMetadata(t *testing.T) {
	f, stdout, _, reg := mailShortcutTestFactory(t)
	reg.Register(&httpmock.Stub{
		Method: "GET",
		URL:    mailboxPath("me", "settings", "auto_reply"),
	...
	if first["file_key"] != "file_ok" || first["data"] != nil || first["error"] != nil {
		t.Fatalf("first image = %#v", first)
	}
	if second["file_key"] != "file_bad" || second["data"] != nil || second["error"] != nil {
		t.Fatalf("second image = %#v", second)
	}
}

func TestMailAutoReplyImageDownloadURL(t *testing.T) {
	f, stdout, _, reg := mailShortcutTestFactory(t)
	reg.Register(&httpmock.Stub{
		Method: "GET",
		...
		Body: map[string]interface{}{
			...
			"data": map[string]interface{}{
				"download_urls": []interface{}{
					map[string]interface{}{"attachment_id": "file_ok", "download_url": "https://download.example/file_ok"},
				},
				"failed_reasons": []interface{}{
					map[string]interface{}{"attachment_id": "file_bad", "reason": "download url not available"},
				},
			},
		},
	})

	if err := runMountedMailShortcut(t, MailAutoReplyImageDownloadURL, []string{"+auto-reply-image-download-url", "--file-keys", "file_ok,file_bad", "--format", "json"}, f, stdout); err != nil {
		t.Fatalf("runMountedMailShortcut() error = %v", err)
	}
	data := decodeShortcutEnvelopeData(t, stdout)
	urls := data["download_urls"].([]interface{})
	if len(urls) != 1 || urls[0].(map[string]interface{})["download_url"] != "https://download.example/file_ok" {
		t.Fatalf("download_urls = %#v", urls)
	}
}
...
func TestMailShortcutsIncludeAutoReply(t *testing.T) {
	want := map[string]bool{
		...
		"+auto-reply-image-download-url": false,
	}
