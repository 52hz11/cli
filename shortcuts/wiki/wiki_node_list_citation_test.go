...
func TestWikiNodeListCitations(t *testing.T) {
	got := wikiNodeListCitations(nil, map[string]interface{}{
	...
	if got[0].URL != "https://applink.feishu.cn/client/wiki/open?wikiToken=wik_node_1" {
...
func TestWikiNodeListMountedExecuteEmitsCitations(t *testing.T) {
	...
	if len(envelope.Data.Nodes) != 3 || envelope.Meta.Count != 3 {
	...
	if len(envelope.Citations) != 2 {
		t.Fatalf("citations = %#v, want 2 entries (missing token dropped)", envelope.Citations)
	}
